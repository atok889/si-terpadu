...
public class DaftarKaryawanDanDosenYangAkanPensiunDAOImpl implements DaftarKaryawanDanDosenYangAkanPensiunDAO {
    ...
    public List<Map> getDaftarDosenDanKaryawanYangAkanPensiun(String tahun) {
        String sqlCreateView = "CREATE OR REPLACE VIEW tempo.pensiun(nama,unit_kerja,umur,pensiun) AS " +
                "SELECT  concat_ws(' ',Gelar_depan,`Nama_peg`,`Gelar_blk`) as Nama_peg,  unkerja.Nama_unit_kerja," +
                " CAST(TIMESTAMPDIFF(YEAR,pegawai.Tgl_lahir, DATE(now())) AS CHAR) as umur," +
                " IF(pegawai.AdmEdu = '1', CAST(56-TIMESTAMPDIFF(YEAR,pegawai.Tgl_lahir, DATE(now())) AS CHAR), " +
                " IF(pegawai.AdmEdu = '2', CAST(65-TIMESTAMPDIFF(YEAR,pegawai.Tgl_lahir, DATE(now())) AS CHAR ),'')) as pensiun " +
...
//
//        System.out.println(sqlCreateView1);
//
